...
import { getUITours } from "../_actions/data"
import { Badge } from "@/components/ui/badge"
import { getTranslations } from "next-intl/server"
import { PaginationLinks } from "@/components/common/pagination"
import { SearchOnlyBox } from "../_components/only-search"
...
export default async function ToursPage({ searchParams }: Props) {
  ...
  const tours = await getUITours(params)
  const t = await getTranslations()

  return (
    <div className='container mx-auto px-4 py-20'>
      <div className='grid grid-cols-2 justify-between items-center w-full mb-4'>
        <h1 className='font-semibold text-3xl'>{t("tours")}</h1>
        <SearchOnlyBox />
      </div>
      {tours?.data.length === 0 && <NoDataLabel />}
      <div className='grid xl:grid-cols-4 gap-4 grid-cols-1 md:grid-cols-3 mb-4'>
        {tours?.data.map((tour) => (
