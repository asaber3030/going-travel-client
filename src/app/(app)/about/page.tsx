    ...
    <div className='container mx-auto px-4 py-12'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16'>
        ...
        <div className='relative h-[400px] rounded-xl overflow-hidden'>
          <Image src='/placeholder.svg?height=800&width=600' alt={t("about_company_image_alt")} fill className='object-cover' />
        </div>
      </div>
      ...
      {/* Team Section */}
      <div className='mb-16'>
        <h2 className='text-3xl font-bold mb-8 text-center text-gray-800'>{t("meet_our_team_title")}</h2>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
          {[1, 2, 3, 4].map((i) => (
            <Card key={i}>
              <CardContent className='p-6 text-center'>
                <div className='relative w-32 h-32 mx-auto mb-4 rounded-full overflow-hidden'>
                  <Image src={`/placeholder.svg?height=200&width=200&text=Team Member ${i}`} alt={t("team_member_image_alt", { index: i })} fill className='object-cover' />
                </div>
                <h3 className='text-xl font-bold mb-1 text-gray-800'>{t("team_member_name", { index: i })}</h3>
                <p className='text-emerald-600 mb-3'>{t("team_member_position")}</p>
                <p className='text-gray-600 text-sm'>{t("team_member_description")}</p>
              </CardContent>
            </Card>
          ))}
        </div>
              ...
              <CardContent className='p-6'>
                ...
                <div className='flex items-center'>
                  <div className='relative w-12 h-12 rounded-full overflow-hidden mr-4'>
                    <Image src={`/placeholder.svg?height=100&width=100&text=C${i}`} alt={t("client_image_alt", { index: i })} fill className='object-cover' />
                  </div>
                  <div>
