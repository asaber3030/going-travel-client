...
export function FilterWithLimousineList({ data }: Props) {
  ...
  const searchFilteredLimousines = limousines.filter((limo) => {
    const matchesSearch = searchQuery === "" || limo.name.toLowerCase().includes(searchQuery.toLowerCase()) || limo.type.toLowerCase().includes(searchQuery.toLowerCase()) || limo.description.toLowerCase().includes(searchQuery.toLowerCase())
        ...
        <div className='lg:col-span-4 space-y-6'>
          <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
            <div>
              ...
              <p className='text-muted-foreground'>
                {t("showing")} {sortedLimousines.length} {t("of")} {data.total} {t("vehicles")}
              </p>
            </div>
            <div className='flex items-center gap-2'>
              <Input placeholder='Search vehicles...' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className='w-full sm:w-auto' />
              ...
              <Select value={sortOption} onValueChange={setSortOption}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value='capacity'>{t("Passenger Capacity")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          {sortedLimousines.length === 0 ? (
            <div className='bg-white rounded-lg shadow-sm border p-8 text-center'>
              ...
              <p className='text-muted-foreground mb-4'>{t("Try adjusting your search criteria")}</p>
              <Button
                variant='outline'
                onClick={() => {
                  setSearchQuery("")
            ...
            <div className='space-y-6 w-full'>
              {sortedLimousines.map((limo: UILimousine, index) => (
                <motion.div key={limo.id} initial='hidden' whileInView='visible' viewport={{ once: true }} variants={fadeInUp} transition={{ delay: index * 0.1 }}>
                        ...
                        <div className='flex flex-col h-full justify-between'>
                          <div>
                            <div className='flex items-start justify-between mb-2'>
                              ...
                              <div className='text-right'>
                                <p className='text-sm text-muted-foreground'>{t("Per hour")}</p>
                            ...
                            <div className='grid grid-cols-3 gap-4 mb-4'>
                              <div className='flex flex-col items-center text-center p-2 bg-muted/30 rounded-lg'>
                                ...
                                <span className='text-sm font-medium'>
                                  {limo.max_passengers} {t("Passengers")}
                                </span>
                              </div>
                              <div className='flex flex-col items-center text-center p-2 bg-muted/30 rounded-lg'>
                          ...
                          <div className='flex items-center justify-end mt-4'>
                            <Button variant='outline' asChild>
                              <Link href={`/limousines/${limo.id}`}>{t("View Details")}</Link>
                            </Button>
                          </div>
                        </div>
                      </CardContent>
