...
import { ApiResponse, APIResponse, ApiError, PaginatedData, Tour, Itinerary, Highlight, Exclusion, Review, TourImage, UpdateTourFormType } from "@/types"
...
import { TourExclusionSchema, TourHighlightSchema, TourItinerarySchema, TourReviewSchema, TourSchema } from "@/schema"
import { z } from "zod"
...
export async function getTours(page: number): Promise<ApiResponse<PaginatedData<Tour>>> {
  try {
    ...
    const sp = build({ page })
    const response = await getRequest<PaginatedData<Tour>>(`/admin/tours` + (sp ? `?${sp}` : ""), loadDefaultHeaders(token, language))
    const data = response
    return data
  } catch (error) {
...
export async function getTour(tourId: number): Promise<Tour | undefined> {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await getRequest<Tour>(`/admin/tours/${tourId}`, loadDefaultHeaders(token, language))
...
export async function getTrashedTours(page: number): Promise<PaginatedData<Tour>> {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await fetch(`${API_URL}/admin/tours/trashed?page=${page}`, loadDefaultHeaders(token, language))
...
export async function createTour(banner: File | null, thumbnail: File | null, data: CreateTourData): Promise<APIResponse<Tour | undefined>> {
  try {
    ...
    formData.append("has_offer", "true")
    ...
    data.translations.map((translation, index) => {
      ...
      formData.append(`translations[${index}][distance_description]`, translation.distance_description)
    })
...
export async function updateTour(tourId: number, banner: File | null, thumbnail: File | null, data: CreateTourData): Promise<APIResponse<Tour | undefined>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function getTourItineraries(tourId: number): Promise<Itinerary[]> {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await getRequest<Itinerary[]>(`/admin/tours/${tourId}/itineraries`, loadDefaultHeaders(token, language))
    const data = response.data
    return data
  } catch (error) {
    return []
...
export async function getTourHighlights(tourId: number): Promise<Highlight[]> {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await getRequest<Highlight[]>(`/admin/tours/${tourId}/highlights`, loadDefaultHeaders(token, language))
    const data = response.data
    return data
  } catch (error) {
    return []
...
export async function getTourExclusions(tourId: number): Promise<Exclusion[]> {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await getRequest<Exclusion[]>(`/admin/tours/${tourId}/inclusions-exclusions`, loadDefaultHeaders(token, language))
    const data = response.data
    return data
  } catch (error) {
    return []
...
export async function getTourReviews(tourId: number): Promise<Review[]> {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await getRequest<Review[]>(`/admin/tours/${tourId}/reviews`, loadDefaultHeaders(token, language))
    const data = response.data
    return data
  } catch (error) {
    return []
...
export async function getTourImages(tourId: number): Promise<TourImage[]> {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await getRequest<TourImage[]>(`/admin/tours/${tourId}/images`, loadDefaultHeaders(token, language))
    const data = response.data
    return data
  } catch (error) {
    return []
...
export async function createTourItinerary(tourId: number, image: File | null, data: CreateTourItieraryData): Promise<ApiResponse<Itinerary>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function updateTourItinerary(itineraryId: number, tourId: number, image: File | null, data: CreateTourItieraryData): Promise<ApiResponse<Itinerary>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function createTourHighlight(tourId: number, image: File | null, data: CreateTourHighlightData): Promise<ApiResponse<Highlight>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function updateTourHighlight(highlightId: number, tourId: number, image: File | null, data: CreateTourHighlightData): Promise<ApiResponse<Highlight>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function createTourExclusion(tourId: number, data: CreateTourExclusionData): Promise<ApiResponse<Exclusion>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function updateTourExclusion(exclusionId: number, tourId: number, data: CreateTourExclusionData): Promise<ApiResponse<Exclusion>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function createTourReview(tourId: number, image: File | null, data: CreateTourReviewData): Promise<ApiResponse<Review>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function updateTourReview(reviewId: number, tourId: number, image: File | null, data: CreateTourReviewData): Promise<ApiResponse<Exclusion>> {
  try {
    const { language, token } = await getDefaultCookies()
...
// Images
export async function createTourImage(tourId: number, image: File | null): Promise<ApiResponse<Review>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function updateTourImage(imageId: number, tourId: number, image: File | null): Promise<ApiResponse<Exclusion>> {
  try {
    const { language, token } = await getDefaultCookies()
...
export async function deleteTourItem(itemId: number, type: string) {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await deleteRequest(`/admin/${type}/${itemId}`, loadDefaultHeaders(token, language))
    const data = response
...
export async function restoreTourItem(itemId: number, type: string) {
  try {
    const { token, language } = await getDefaultCookies()
    const response = await postRequest(`/admin/${type}/${itemId}/restore`, {}, loadDefaultHeaders(token, language))
    const data = response
    revalidatePath(routes.tours.changeDetails(itemId, type))
    return data
  } catch (error) {
