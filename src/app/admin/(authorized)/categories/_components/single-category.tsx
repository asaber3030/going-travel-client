...
import { deleteCategory } from "../_helpers/actions";
...
export const SingleCategoryComponent = ({ categoryData }: Props) => {
  ...
  const activeTranslation = categoryData.translations?.find((t) => t.locale === activeTab) ?? {
    locale: "en",
    name: "N/A",
    description: "N/A"
  };
                ...
                <div>
                  ...
                  <div className='flex flex-wrap gap-2 mb-4'>
                    {categoryData.translations?.map((translation) => (
                      ...
                      >
                        {Languages.find((lang) => lang.code === translation.locale)?.name}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
                    <TabsList className='hidden'>
                      {categoryData.translations?.map((translation) => (
                        <TabsTrigger key={translation.locale} value={translation.locale}>
                          {Languages.find((lang) => lang.code === translation.locale)?.name}
                        </TabsTrigger>
                      ))}
                    </TabsList>
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className='text-base'>
                            {Languages.find((lang) => lang.code === activeTranslation.locale)?.name}
                            Translation
                          </CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className='space-y-4'>
                            <div>
                              ...
                              <p className='text-lg font-medium'>{activeTranslation.name}</p>
                            </div>
                            <div>
                              <h3 className='text-sm font-medium text-muted-foreground'>
                                Description
                              </h3>
                              <p className='whitespace-pre-wrap'>{activeTranslation.description}</p>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
